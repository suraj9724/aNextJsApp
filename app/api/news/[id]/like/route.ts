...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        const updatedNews = await News.findById(params.id).select('likes dislikes likedBy dislikedBy'); // Select only relevant fields
        ...
        return NextResponse.json({ message: 'Error liking news item', error: err.message }, { status: 500 });
    }
} 